...
class ServerTests(unittest.TestCase):
    ...
    def test_index_served(self):
        ...
        self.assertIn(b"Attach as NFT", body)
        self.assertIn(b'id="canvasBar"', body)
        ...
        self.assertNotIn(b'id="sd-bobble"', body)
        self.assertNotIn(b'id="sdBobbleDisplace"', body)
        self.assertIn(b'id="bobbleBtn"', body)
        self.assertIn("25× slideshow on / off".encode(), body)
        self.assertIn("25× catalog slideshow".encode(), body)
    ...
    def test_post_saves_plate_and_caption(self):
        ...
        self.assertTrue(data["src"].startswith("/media/posts/"))

        status, ctype, listed = self._get("/api/posts")
        self.assertEqual(status, 200)
        self.assertIn("application/json", ctype)
        board = json.loads(listed)
        self.assertTrue(board["ok"])
        self.assertEqual(board["count"], 1)
        self.assertEqual(board["posts"][0]["caption"], "Golden hour")
        self.assertEqual(board["posts"][0]["file"], data["file"])
        self.assertEqual(board["posts"][0]["src"], data["src"])

        status, ctype, image = self._get("/media/posts/" + data["file"])
        self.assertEqual(status, 200)
        self.assertTrue(ctype.startswith("image/"))
        self.assertEqual(image, TINY_PNG)
